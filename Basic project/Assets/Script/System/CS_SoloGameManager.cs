...
public class CS_SoloGameManager : MonoBehaviour
{
    [Header("Game Object")]
    ...
    private static CS_SoloGameManager _instance;

    public static CS_SoloGameManager Instance
    {
        get
        {
            if (_instance == null)
            {
                _instance = FindObjectOfType<CS_SoloGameManager>();

                if (_instance == null)
                {
                    GameObject singletonObject = new GameObject();
                    _instance = singletonObject.AddComponent<CS_SoloGameManager>();
                    singletonObject.name = typeof(CS_SoloGameManager).ToString() + " (Singleton)";
    ...
    private void Awake()
    {
        if (_instance == null)
        {
            _instance = this as CS_SoloGameManager;
